$(document).ready(function () {
  ...
  // Display Save button if any setting field is changed
    $("[id^=Sushi_]").change(function () {
        if ( $('#Inst').val() == 1 ) {     // Hide if Inst == all consortium
            $('#SaveSushi').hide();
        } else {
            $('#SaveSushi').show();
        }
  ...
  // Institution change (re)builds sushi settings
    $("#Inst").change(function () {
 // change function of listbox
        $('#notice').hide();      // hide notice when Inst changes
        if ( $('#Inst').val() == 1 ) {  // 1=all consortium, not zero
            $('#SaveSushi').hide();   // hide Button if Prov is reset
        } else {
            $('#SaveSushi').show();   // display Button when Inst is set
            $.get(
